...
const startIdx = lines.findIndex(line => line.includes('{/* Territories (Convex Hulls) */}'));
const endIdx = lines.findIndex((line, idx) => idx > startIdx && line.includes('</Polyline>') && lines[idx+1].includes(');') && lines[idx+2].includes('})}'));
...
if (startIdx !== -1 && endIdx !== -1) {
                                        ...
                                        >
                                            {currentZoom >= 14 && (
                                                <Tooltip permanent direction="center" className={\`bg-white/95 border-2 shadow-xl text-slate-800 text-xs sm:text-sm px-3 py-2 rounded-2xl backdrop-blur-md z-[1000] transition-opacity duration-300 \${isDimmed ? 'opacity-30' : 'opacity-100'}\`} opacity={1} interactive={true}>
                                                    <div className="text-center leading-tight min-w-[80px]" onClick={(e) => { L.DomEvent.stopPropagation(e.originalEvent); setFocusedTerritoryId(t.workerId); }}>
    ...
    lines.splice(startIdx, endIdx - startIdx + 3, replacement);
