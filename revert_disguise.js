...
m = m.replace(/user\.role === 'super_admin' \? '#eab308' : user\.role === 'admin' \? '#7c4dff' : '#444'/g, `user.role === 'admin' ? '#7c4dff' : '#444'`);
m = m.replace(/user\.role === 'super_admin' \? '최고관리자' : user\.role === 'admin' \? '관리자' : '청소부'/g, `user.role === 'admin' ? '관리자' : '청소부'`);
...
u = u.replace(/user\.role === 'super_admin' \? '#eab308' : user\.role === 'admin' \? '#7c4dff' : '#444'/g, `user.role === 'admin' ? '#7c4dff' : '#444'`);
u = u.replace(/user\.role === 'super_admin' \? '최고관리자' : user\.role === 'admin' \? '관리자' : '청소부'/g, `user.role === 'admin' ? '관리자' : '청소부'`);
fs.writeFileSync('src/app/admin/users/user-management.tsx', u);

console.log("Reverted user list disguises");
